$.fn.adminUImenu = function( settings ) {
	var oid = this.attr("id");
    // adminUI Menu default settings:
    var defaults = {
	...
	var adminUIMenu = {
        adminUIMenuInit: function() {
			adminUIMenu.Handlethemelayout();
			adminUIMenu.HandleverticalMenuplacement();
			adminUIMenu.HandlehorizontalMenuplacement();
			adminUIMenu.HandleMenulayout();
			adminUIMenu.HandleDeviceType();
			adminUIMenu.Handlecomponetheight();
			adminUIMenu.HandleMenuOnClick();
			adminUIMenu.HandleMenuTrigger();
			adminUIMenu.HandleSubMenuTrigger();
			adminUIMenu.HandleActiveItem();
			adminUIMenu.HandleOffcanvasMenu();
			adminUIMenu.HandleVerticalLeftHeader();
			adminUIMenu.HandleThemeBackground();
			adminUIMenu.HandleActiveItemStyle();
			adminUIMenu.HandleItemBorder ();
			adminUIMenu.HandleBorderStyle ();
			adminUIMenu.HandleSubItemBorder ();
			adminUIMenu.HandleDropDownIconStyle ();
			adminUIMenu.HandleOptionSelectorPanel ();
			adminUIMenu.HandleNavbarPosition ();
			adminUIMenu.HandleVerticalSubMenuItemIconStyle ();
			adminUIMenu.HandleVerticalNavigationView ();
			adminUIMenu.HandleHorizontalItemIsCentered();
			adminUIMenu.HandleHorizontalItemAlignment ();
			adminUIMenu.HandleSubMenuOffset();
			adminUIMenu.HandleHorizontalStickyNavigation(); 
			adminUIMenu.HandleDocumentClickEvent(); 
			adminUIMenu.HandleVerticalScrollbar();
			adminUIMenu.HandleHorizontalMobileMenuToggle();
			adminUIMenu.horizontalNavigationMenuIcon();
			adminUIMenu.verticalNavigationSearchBar();
			adminUIMenu.safariBrowsercompatibility();
		...
		verticalNavigationSearchBar: function() {
			if (settings.themelayout === "vertical"){ 
				$('.searchbar-toggle').on('click', function( ){ 
					$(this).parent('.adminUI-search').toggleClass('open');
				});
			}  
		},
		horizontalNavigationMenuIcon: function() {
			if (settings.themelayout === "horizontal"){ 
				switch(settings.horizontalNavigationMenuIcon){
					case false: 
						$('#'+oid + '.adminUI .adminUI-navbar .adminUI-item > li > a .adminUI-micon').hide(); 
						$('#'+oid + '.adminUI .adminUI-navbar .adminUI-item.adminUI-search-item > li > a .adminUI-micon').show(); 
					break; 
					default:  
				}
			}  
		...
		HandleHorizontalMobileMenuToggle: function() {
			if (settings.themelayout === "horizontal"){
				$('.adminUIbrand-xs .menu-toggle a').on('click', function( ){
					$('.adminUI-navbar').toggleClass('show-menu')
				});
			}  
		},
				...
				if (satnt === "expanded" || satnt === "compact"){
					...
					if (mt === "click"){
						$(window).on("load",function(){
							$(".adminUI-navbar").mCustomScrollbar({
								axis:"y",  
		...
		HandleDocumentClickEvent: function() {
			function closeSubMenu() {
				$(document).on('click', function(evt) {
					...
					var sdt = $('#'+oid).attr('adminUI-device-type');
					var vnt = $('#'+oid).attr('vertical-nav-type');
					var el = $('#'+oid + ' .adminUI-item li');
					if (!target.parents('.adminUI-item').length) {
						if( sdt != "phone"){
							if( vnt != "expanded"){
								el.removeClass('adminUI-trigger'); 
							} 
						}  
					}  
				});
			...
			function closeLeftbarSearch() {
				$(document).on('click', function(evt) {
					var target = $(evt.target); 
					var el = $('#'+oid + ' .adminUI-search');
					if (!target.parents('.adminUI-search').length) {
						el.removeClass('open'); 
					} 
			...
			switch(settings.horizontalstickynavigation){
				case true:
					$(window).on('scroll',function() {
						...
						if(scrolltop >= 100) { 
							$('.adminUI-navbar').addClass('stickybar');
							$('stickybar').fadeIn(3000);
						}
						
						else if(scrolltop <= 100) {
							$('.adminUI-navbar').removeClass('stickybar')
		...
		HandleSubMenuOffset: function() { 
            switch(settings.themelayout){
				case 'horizontal':
				   ...
				   if(trigger === "hover" ) {
					  $("li.adminUI-hasmenu").on('mouseenter mouseleave', function (e) {
							if ($('.adminUI-submenu', this).length) { 
								var elm = $('.adminUI-submenu:first', this);
								var off = elm.offset();
								var l = off.left;
								var w = elm.width();
						 ...
						 $("li.adminUI-hasmenu").on('click', function (e) {
							 e.preventDefault();
							if ($('.adminUI-submenu', this).length) { 
								var elm = $('.adminUI-submenu:first', this);
								var off = elm.offset();
		...
		HandleHorizontalItemIsCentered: function() {
			if (settings.themelayout === "horizontal"){
				switch(settings.horizontalNavIsCentered){
					case true:
						$('#'+oid + ' .adminUI-navbar').addClass("isCentered");
					break;
					case false:
						$('#'+oid + ' .adminUI-navbar').removeClass("isCentered");
					break;                                          
					default:                                         
				}
			}
		...
		HandleHorizontalItemAlignment: function() {
			...
			if(layout === "horizontal" ) {
				function branditemalignment() { 
					var elm = $('#'+oid + '.adminUI .adminUI-navbar .adminUI-brand');
					if (settings.horizontalBrandItem === true){
						
						switch(settings.horizontalBrandItemAlign){
							case 'left': 
								elm.removeClass('adminUI-right-align');
								elm.addClass('adminUI-left-align');
							break;
							case 'right': 
								elm.removeClass('adminUI-left-align');
								elm.addClass('adminUI-right-align');
							break;                                          
							default:                                         
						} 
					}else {
				...
				function leftitemalignment() { 
					var elm = $('#'+oid + '.adminUI .adminUI-navbar .adminUI-item.adminUI-left-item');
				...
				function rightitemalignment() {
					var elm = $('#'+oid + '.adminUI .adminUI-navbar .adminUI-item.adminUI-right-item');
				...
				function searchitemalignment() {
					var elm = $('#'+oid + '.adminUI .adminUI-navbar .adminUI-search-item');
		...
		HandleVerticalNavigationView: function() { 
				...
				case 'vertical':
					...
					$('#'+oid + '.adminUI').attr("vnavigation-view", ev );
				break;
				case 'horizontal':
					var ev = settings.horizontalNavigationView;
					$('#'+oid + '.adminUI').attr("hnavigation-view", ev );
				break;                                          
				default:                                         
				}  
        },
		HandleVerticalSubMenuItemIconStyle: function() { 
				...
				case 'vertical':
					...
					$('#'+oid + ' .adminUI-navbar .adminUI-hasmenu').attr("subitem-icon", ev );
				break;
				case 'horizontal':
					 $('#'+oid + ' .adminUI-navbar .adminUI-hasmenu').attr("subitem-icon", ev );
				break;                                          
		...
		HandleNavbarPosition: function() { 
				...
				case 'vertical':
					if ( navposition == true){
						$('#'+oid + ' .adminUI-navbar').attr("adminUI-navbar-position", 'fixed' );
						$('#'+oid + ' .adminUI-header .adminUI-left-header').attr("adminUI-lheader-position", 'fixed' );
					}else {
						$('#'+oid + ' .adminUI-navbar').attr("adminUI-navbar-position", 'absolute' ); 
						$('#'+oid + ' .adminUI-header .adminUI-left-header').attr("adminUI-lheader-position", 'absolute' );
					}
					if ( headerposition == true){
						$('#'+oid + ' .adminUI-header').attr("adminUI-header-position", 'fixed' );
						$('#'+oid + ' .adminUI-main-container').css('margin-top', $(".adminUI-header").outerHeight());
						
					}else {
						$('#'+oid + ' .adminUI-header').attr("adminUI-header-position", 'relative' );
						$('#'+oid + ' .adminUI-main-container').css('margin-top', '0px');
					}
				break;
				case 'horizontal':
					if ( navposition == true){
						$('#'+oid + ' .adminUI-navbar').attr("adminUI-navbar-position", 'fixed' );
						$('#'+oid + ' .adminUI-header').attr("adminUI-header-position", 'fixed' );
						$('#'+oid + ' .adminUI-navbar').css('margin-top', $(".adminUI-header").outerHeight());
					}else{
						$('#'+oid + ' .adminUI-navbar').attr("adminUI-navbar-position", ' ' );
						$('#'+oid + ' .adminUI-header').attr("adminUI-header-position", 'relative' );
						$('#'+oid + ' .adminUI-navbar').css('margin-top', '0px');
					}
				break;                                          
				default:                                         
				}  
		...
		HandleDropDownIconStyle: function() { 
			...
			switch(settings.themelayout){
				case 'vertical':
					$('#'+oid + ' .adminUI-navbar .adminUI-hasmenu').attr("dropdown-icon", ev );
				break;
				case 'horizontal':
					 $('#'+oid + ' .adminUI-navbar .adminUI-hasmenu').attr("dropdown-icon", ev );
				break;                                          
		...
		HandleSubItemBorder: function() { 
				...
				case true:
					$('#'+oid + ' .adminUI-navbar .adminUI-item').attr("subitem-border","true" );
				break;
				case false:
					$('#'+oid + ' .adminUI-navbar .adminUI-item').attr("subitem-border","false" );
				break;                                          
				default:                                         
				}  
        },
		HandleBorderStyle: function() { 
				...
				case true:
					$('#'+oid + ' .adminUI-navbar .adminUI-item').attr("item-border-style", ev );
				break;
				case false:
					$('#'+oid + ' .adminUI-navbar .adminUI-item').attr("item-border-style", "" );
				break;                                          
				default:                                         
				}  
        },
		HandleItemBorder: function() { 
            switch(settings.ItemBorder){
				case true:
					$('#'+oid + ' .adminUI-navbar .adminUI-item').attr("item-border","true" );
				break;
				case false:
					$('#'+oid + ' .adminUI-navbar .adminUI-item').attr("item-border","false" );
				break;                                          
				default:                                         
			}  
        },
		HandleActiveItemStyle: function() { 
				...
				if(ev != undefined && ev != "") {
					$('#'+oid + ' .adminUI-navbar').attr("active-item-style", ev);
				} else {
					$('#'+oid + ' .adminUI-navbar').attr("active-item-style", "style0");
				}
        },
		HandleThemeBackground: function() {
             ...
             function setheadertheme() {
				...
				if(ev != undefined && ev != "") {
					$('#'+oid + ' .adminUI-header').attr("header-theme", ev);
				} else {
					$('#'+oid + ' .adminUI-header').attr("header-theme", "theme1");
				}
            };
            function setlheadertheme() {
                var ev = settings.LHeaderBackground;
				if(ev != undefined && ev != "") {
					$('#'+oid + ' .adminUI-header .adminUI-left-header').attr("lheader-theme", ev);
				} else {
					$('#'+oid + ' .adminUI-header .adminUI-left-header').attr("lheader-theme", "theme1");
				}
            };
            function setnavbartheme() {
                var ev = settings.NavbarBackground;
				if(ev != undefined && ev != "") {
					$('#'+oid + ' .adminUI-navbar').attr("navbar-theme", ev);
				} else {
					$('#'+oid + ' .adminUI-navbar').attr("navbar-theme", "theme1");
				}
            };
			function setactiveitemtheme() {
                var ev = settings.ActiveItemBackground;
				if(ev != undefined && ev != "") {
					$('#'+oid + ' .adminUI-navbar').attr("active-item-theme", ev);
				} else {
					$('#'+oid + ' .adminUI-navbar').attr("active-item-theme", "theme1");
				}
            };
			function setsubitemtheme() {
                var ev = settings.SubItemBackground;
				if(ev != undefined && ev != "") {
					$('#'+oid + ' .adminUI-navbar').attr("sub-item-theme", ev);
				} else {
					$('#'+oid + ' .adminUI-navbar').attr("sub-item-theme", "theme1");
				}
            };
		...
		HandleVerticalLeftHeader: function() { 
			...
			switch(settings.collapseVerticalLeftHeader){
				case true:
					$('#'+oid + ' .adminUI-header').addClass('iscollapsed');
					$('#'+oid + ' .adminUI-header').removeClass('nocollapsed');
					$('#'+oid + '.adminUI').addClass('iscollapsed');
					$('#'+oid + '.adminUI').removeClass('nocollapsed');
					
				   /*  $('#'+oid + ' .adminUI-header.nocollapsed .adminUI-left-header').css('width', '');  */
				break;
				case false:  
					$('#'+oid + ' .adminUI-header').removeClass('iscollapsed');
					$('#'+oid + ' .adminUI-header').addClass('nocollapsed');
					$('#'+oid + '.adminUI').removeClass('iscollapsed');
					$('#'+oid + '.adminUI').addClass('nocollapsed');
				   /*  $('#'+oid + ' .adminUI-header.nocollapsed .adminUI-left-header').css('width', $(".adminUI-navbar").width());  */
				break;                                          
				default:                                         
				}
			}else {
		...
		HandleActiveItem: function() {  
			switch(settings.activeMenuClass){
				case  "active":
					$('li:not("li.adminUI-hasmenu")').on( 'click', function () {
					var str = $(this).closest('.adminUI-submenu').length;
						if (str === 0){  
								$(this).closest('.adminUI-inner-navbar').find('li.active').removeClass('active');
							...
							if($(this).hasClass('active')){
								...
								$(this).closest('.adminUI-inner-navbar').find('li.active').removeClass('active');
								$(this).parents('.adminUI-hasmenu').addClass('active');
								$(this).addClass('active');
							}
						}
				});
				break;
				case  false:
					$('.adminUI-header').removeClass(settings.navbbgclass); 
				break;
				default:  
			}                                                                                  
        },
		HandleSubMenuTrigger: function(){
				...
				case 'hover': 
				$('#'+oid + ' .adminUI-navbar .adminUI-hasmenu').addClass('is-hover');
					 // Initialize
					var $window = $(window);
					var $dropdown = $('.adminUI-submenu > li');
					...
					function bindTwo (eventType) {
						if (eventType == 'hover') {
							...
							$dropdown.off('click').off('mouseenter mouseleave').hover(
								function() {
									$(this).addClass('adminUI-trigger'); 
								},
								function() {
									$(this).removeClass('adminUI-trigger');
								}
							);
						}
							...
							$dropdown.off('mouseenter mouseleave').off('click').on('click',
								function(e) {
									...
									var str = $(this).closest('.adminUI-submenu').length; 
									if (str === 0){
										if($(this).hasClass('adminUI-trigger')){
											$(this).removeClass('adminUI-trigger');
										}else{
											$(this).closest('.adminUI-inner-navbar').find('li.adminUI-trigger').removeClass('adminUI-trigger');
											$(this).addClass('adminUI-trigger');
										}
									}else{
										if($(this).hasClass('adminUI-trigger')){
											...
											$(this).closest('.adminUI-submenu').find('li.adminUI-trigger').removeClass('adminUI-trigger');
											$(this).addClass('adminUI-trigger');
										}
									}
								}
				...
				break; 
				case 'click': 
				$('#'+oid + ' .adminUI-navbar .adminUI-hasmenu').removeClass('is-hover');
					$(".adminUI-submenu > li").on( 'click', function (e) {
						e.stopPropagation();
					var str = $(this).closest('.adminUI-submenu').length; 
						if (str === 0){
							if($(this).hasClass('adminUI-trigger')){
		...
		HandleMenuTrigger: function(){ 
			...
			switch(settings.MenuTrigger){
				case 'hover':
				$('#'+oid + ' .adminUI-navbar').addClass('is-hover');
					 // Initialize
					var $window = $(window);
					var $dropdown = $(".adminUI-item > li");
					var currentSize = $window.width();
					var currentEvent = '';
					// Attach current event on load
				...
				break; 
				case 'click':
				$('#'+oid + ' .adminUI-navbar').removeClass('is-hover');
					$(".adminUI-item > li ").on( 'click', function () {
						if($(this).hasClass('adminUI-trigger')){
							$(this).removeClass('adminUI-trigger');
						}else{
							$(this).closest('.adminUI-inner-navbar').find('li.adminUI-trigger').removeClass('adminUI-trigger');
		...
		HandleMenuOnClick: function() {			
			...
			if (settings.themelayout === "vertical" ) {
				$('.sidebar_toggle a, .adminUI-overlay-box').on("click", function() {
					$(this).parent().find('.menu-icon').toggleClass("is-clicked");
					var dt = $('#'+oid).attr("adminUI-device-type");
					if ( dt == "desktop" ){
					...
					$('.adminUI').addClass("adminUI-toggle-animate"); 
					setTimeout(function(){
						 $('.adminUI').removeClass("adminUI-toggle-animate");  
					}, 250);		
				});
			}else if (settings.themelayout === "horizontal" )  { 
				if (totalwidth >= 768 && totalwidth <= 1024) {
						$('#'+oid).attr("adminUI-device-type", "tablet");
				} else if (totalwidth < 768) {
						$('#'+oid).attr("adminUI-device-type", "phone");
				} else { 
						$('#'+oid).attr("adminUI-device-type", "desktop");
				}
			}
		},
		Handlecomponetheight: function() {
			function setHeight() {
                ...
                var HH = $(".adminUI-header").innerHeight();
                var NH = $(".adminUI-navbar").innerHeight();
				var FH = $(".adminUI-footer").innerHeight();
                var contentHH = WH - HH;
				var contentVH = WH - HH ;
				var lpanelH = WH - HH ;
				if (settings.themelayout === "horizontal" ) {  
					$(".adminUI-main-container").css('min-height', contentHH);
				} else if (settings.themelayout === "vertical" ) {
					if ( contentVH >= lpanelH ){
						$(".adminUI-main-container").css('min-height', contentVH); 
					}else {
						$(".adminUI-main-container").css('min-height', lpanelH); 
					}
				} else {
					return false;
				}                   
		...
		HandleDeviceType: function() {			
            ...
            devicesize();
             ...
             $(window).resize(function() { 
				...
				dt = $('#'+oid).attr('adminUI-device-type')
